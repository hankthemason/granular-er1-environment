...
class Led {
        ...
        for (let y = 0; y < 8; y++) {
          ...
          for (let x = pageStart; x < pageStart + 16; x++) {
            ...
            if (
              x < pageEnd &&
              seq[x].on &&
              currentTrack.instrumentConfig.mapping[y + yOffset] &&
              seq[x].pitches[y + yOffset] ===
                currentTrack.instrumentConfig.mapping[y + yOffset]
            ) {
              row.push(1);
            } else {
            ...
            for (let x = pageStart; x < pageStart + 16; x++) {
              if (x < pageEnd && seq[x].on && seq[x].pitches.length) {
                ...
                for (const pitch of seq[x].pitches) {
                  const octaveNote =
                    pitch ===
                    (currentTrack.sequence[x].octave + 1) * 12 +
                      currentTrack.rootNote;
                  noteTranslated = octaveNote
                    ? 7
                    : currentTrack.scale.indexOf(
                        (pitch - currentTrack.rootNote) % 12
                      );
                  if (noteTranslated === y) {
                    row[x] = 1;
                  }
      ...
      for (let y = 0; y < 8; y++) {
        ...
        for (let x = pageStart; x < pageStart + 16; x++) {
          ...
          if (x < pageEnd && seq[x].on && seq[x].pitch) {
            ...
            const octaveNote =
              currentTrack.sequence[x].pitch ===
              (currentTrack.sequence[x].octave + 1) * 12 +
                currentTrack.rootNote;
            noteTranslated = octaveNote
              ? 7
              : currentTrack.scale.indexOf(
                  (currentTrack.sequence[x].pitch - currentTrack.rootNote) % 12
                );
          }
    ...
    if (currentTrack.sequence[x].on && x < currentTrack.upperLimit) {
      ...
      //monoTrack
        ...
        if (currentTrack.sequence[x].pitch) {
          ...
          const octaveNote =
            currentTrack.sequence[x].pitch ===
            (currentTrack.sequence[x].octave + 1) * 12 + currentTrack.rootNote;
  ...
  buildColumnTop = (col, x, currentTrack) => {
    ...
    col[1] =
      currentTrack.sequence[x].slide && currentTrack.sequence[x].on ? 1 : 0;
    col[0] =
      currentTrack.sequence[x].noteEffect && currentTrack.sequence[x].on
        ? 1
        : 0;
    return col;
  };
