...
class Track {
  ...
  updateNoteValue = (newNoteValue) => {
    ...
    maxApi.outlet(
      "changeNoteValue",
      noteValues[this.noteValue].coeff,
      this.track
    );
  };
...
class MonoTrack extends Track {
  ...
  updateStepPitch = (step, y) => {
    ...
    if (
      note >= this.instrumentConfig.minNote &&
      note <= this.instrumentConfig.maxNote
    ) {
  ...
  updateStepOctave = (step, y) => {
    ...
    const calculatedOctaveSpan = Math.ceil(
      (this.instrumentConfig.maxNote - this.rootNote) / 12
    );
    if (15 - y < calculatedOctaveSpan) {
      ...
      this.sequence[step].octave > prevOctave
        ? (this.sequence[step].pitch += diff * 12)
        : (this.sequence[step].pitch -= diff * 12);
    }
  };
  ...
  getNotes = (step) => {
    ...
    const msPerNote = this.sequence[step].slide
      ? ms + ms * 0.5
      : ms - ms * 0.25;
    ...
    const passNote =
      this.sequence[step].prob < 8
        ? notePasses(this.sequence[step].prob)
        : true;
    ...
    if (this.sequence[step].on && passNote) {
      return [
        randomNote || this.sequence[step].pitch,
        velocity,
        msPerNote,
        noteEffectType,
        this.sequence[step].noteEffect,
      ];
    } else {
      return [null, null];
    }
...
class MappedTrack extends PolyTrack {
  ...
  updateStepPitch = (step, y) => {
    ...
    const noteIdx = 15 - y + yOffset;
    const numVCOS = 4;
    const isVCO = this.sequence[step].octave === 0 && noteIdx < numVCOS;
    if (this.mode === "normal") {
      //turn note off if already selected
      if (
        this.sequence[step].pitches.includes(
          this.instrumentConfig.mapping[noteIdx]
        )
      ) {
      ...
      //otherwise turn note on and make velocity, prob max (8)
        ...
        this.sequence[step].pitches[noteIdx] =
          this.instrumentConfig.mapping[noteIdx];
        this.sequence[step].velocity = 8;
        this.sequence[step].prob = 8;
      }
    } else {
      //this mode only effects the bottom four notes
      ...
      if (
        ...
        if (isVCO) {
          for (let i = 0; i < numVCOS; i++) {
            this.sequence[step].pitches[i] = null;
          }
        } else {
          this.sequence[step].pitches[15 - y + yOffset] = null;
        }
      ...
      //otherwise turn note on and make velocity, prob max (8)
        ...
        if (isVCO) {
          for (let i = 0; i < numVCOS; i++) {
            this.sequence[step].pitches[i] = this.instrumentConfig.mapping[i];
          }
        } else {
          this.sequence[step].pitches[noteIdx] =
            this.instrumentConfig.mapping[noteIdx];
  ...
  getNotes = (step) => {
    ...
    if (this.sequence[step].on && passNote) {
      const notes = this.sequence[step].pitches.filter(
...
class ScalarPolyTrack extends PolyTrack {
  ...
  updateStepPitch = (step, y) => {
    ...
    if (this.pitchViewCoupledToOctave) {
      ...
      let note =
        this.rootNote + this.scale[scalarIdx % 7] + stepObj.octave * 12;
  ...
  getNotes = (step) => {
    ...
    const pitches = notesAreRandom
      ? getRandomNotes(this)
      : this.sequence[step].pitches.filter((pitch) => pitch >= 0);

    if (
      this.sequence[step].on &&
      this.sequence[step].pitches.length &&
      passNote
    ) {
