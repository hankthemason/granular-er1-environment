...
const handleDeleteSequence = require("./utils/handleDeleteSequence");
...
const handleClearAll = require("./utils/handleClearAll");
...
const main = async () => {
  ...
  let masterConfig = new MasterConfig(
    tracks,
    null,
    masterHz,
    syncing,
  ...
  const initialize = async () => {
    ...
    tracks.forEach((track) => {
      ...
      //this ensures that Max gets each track's initial note value and updates phasors accordingly
      track.updateNoteValue(track.noteValue);
  ...
  async function run() {
    grid.key((x, y, s) => {
      ...
      if (y === 0 && x < 3) {
        handleClearAll(x, s, masterConfig, tracks, currentTrack, led, grid);
      }

      if (y === 0 && x < 6 && x === currentTrack.track) {
        handleDeleteSequence(s, masterConfig, currentTrack, led, grid);
      }
      ...
      if (s === 1) {
        ...
        //toggle universal note value
          ...
          masterConfig.universalNoteValueOn =
            !masterConfig.universalNoteValueOn;
  ...
  maxApi.addHandler("tick", (track) => {
    ...
    let step = t.step;
    if (t.track === 0 && step === 0) {
      maxApi.outlet("changeChord", "bang");
    }
    ...
    const [notes, velocity, msPerNote, noteEffectType, noteEffect] =
      t.getNotes(step);
    if (masterConfig.swing && t.isMaster) {
      masterConfig.delayOn =
        (tracks[masterConfig.masterTrack].step + 1) % 2 !== 0 ? 0 : 1;
    }
    //poly
    if (notes && t.poly) {
      maxApi.outlet(
    ...
    if (t.track === 0 && step === 0) {
      maxApi.outlet("track0Start", "bang");
    }
      ...
      for (let x = pageStart; x < pageEnd; x++) {
        if (step === x) {
          ...
          if (t.view === 2 || t.view === 3) {
            if (
              t.sequence[step].velocity === 8 ||
              t.sequence[step].prob === 8
            ) {
              for (let y = 8; y < 16; y++) {
                led.grid[y][x % 16] = 0;
              }
  ...
  maxApi.addHandler("readSeq", (path) => {
    ...
    maxApi.outlet(
      "readSeq",
      masterConfig.masterHz,
      masterConfig.swing,
      masterConfig.swingAmount
    );
  });

  maxApi.addHandler("setEr1Mode", (modeIdx) => {
    tracks[0].setMode(modeIdx);
  });
